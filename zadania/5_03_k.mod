...
var termin_zakonczenia{zadania} >= 0;
# Zapas czasu dla każdego zadania         
...
o_poprzednika{(i,j) in poprzedniki}: termin_zakonczenia[j] >= termin_zakonczenia[i] + czas_wykonania[i] - zapas_czasu[i]; 
# Ograniczenie maksymalnego terminu zakończenia projektu
o_maksymalny_termin{i in zadania}: termin_zakonczenia[i] + czas_wykonania[i] - zapas_czasu[i] <= maksymalny_czas;   
# Zapewnienie minimalnego czasu trwania dla każdego zadania
o_minimalny_czas_wykonania{i in zadania}: czas_wykonania[i] - zapas_czasu[i] >= minimalny_czas[i];  	
