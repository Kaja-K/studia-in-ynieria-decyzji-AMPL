...
# Ograniczenia
o_budzetowe{j in 1..lata}: sum{i in 1..projekt} pieniadze[j,i] <= budzet[j]; 						# Suma pieniędzy zainwestowanych we wszystkie projekty nie może przekroczyć budżetu na dany rok.
o_ukonczenie{i in 1..projekt}: ukonczenie[i] <= (sum{j in 1..lata} pieniadze[j,i]) / koszt[i] <= 1; # Suma pieniędzy zainwestowanych w projekt do momentu końca jego realizacji podzielona przez koszt projektu musi wynosić od 0 do 1.
o_inwestowanie{j in 1..lata, i in 1..projekt}: pieniadze[j,i] = pieniadze[j,i] * inwestowanie[j,i]; # Zainwestowane pieniądze w projekty w danym roku zależą od możliwości inwestowania w dane projekty w danym roku.
