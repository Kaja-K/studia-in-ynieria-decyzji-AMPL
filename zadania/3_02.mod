...
# Ograniczenia
o_pracy: sum{z in zboze} praca[z] * ilosc_hektarow[z] <= limit_pracy; 	# Określa, że suma iloczynów wymaganego czasu pracy i ilości obsianych hektarów dla każdego zboża nie może przekroczyć limitu czasu pracy.
o_popyt{z in zboze}: ilosc_hektarow[z] * wydajnosc[z] <= popyt[z]; 		# Określa, że iloczyn ilości obsianych hektarów i wydajności każdego zboża nie może przekroczyć popytu na to zboże
o_pole: sum{i in zboze} ilosc_hektarow[i] <= rozmiar_pola; 				# Określa, że suma ilości obsianych hektarów dla każdego zboża nie może przekroczyć rozmiaru pola.

data;
param rozmiar_pola := 100;
param limit_pracy := 1400;
param: zboze: popyt cena wydajnosc praca :=  	"Zyto" 		560   300 	10 		12
