...
o_dostepnej_liczby{i in samoloty}:sum{j in trasy} samoloty_na_trasie[i,j] <= dostepne_samoloty[i];
...
o_pojemnosci{j in trasy}:sum{i in samoloty} (liczba_lotow[i,j] * pojemnosc_samolotow[i] * samoloty_na_trasie[i,j]) = liczba_pasazerow[j] - nadwyzka_pasazerow[j] + niedobor_pasazerow[j]; 
