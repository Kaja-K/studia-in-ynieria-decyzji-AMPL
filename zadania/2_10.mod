...
# Zmienne decyzyjne
var produkcja{1..okres} >= minimalna_produkcja, <= maksymalna_produkcja;# Produkcja w okresie t
var magazyn{1..okres} >= 0;                 							# Magazyn w okresie t
var sprzedane{1..okres} >= 0;               							# Sprzedane w okresie t

# Funkcja celu -  Maksymalizacja zysku (dochod ze sprzedazy - koszty produkcji - koszty magazynowania - koszty sprzedanego popytu)
maximize zysk: sum{t in 1..okres} (cena_sprzedazy[t] * sprzedane[t] 	- 
...
# Ograniczenia
o_popytu{t in 1..okres}: sprzedane[t] <= oszacowany_popyt[t]; 															# Gwarantuje, że ilość sprzedanych produktów w danym okresie nie przekroczy oszacowanego popytu. 
o_magazynu: magazyn[1] = produkcja[1] - sprzedane[1];     																# Określa stan początkowy magazynu jako różnicę między produkcją a sprzedażą w pierwszym okresie. 
o_rownowagi{t in 2..okres}: magazyn[t] = magazyn[t-1] + produkcja[t] - sprzedane[t];    								# Zapewnia równowagę magazynową, gdzie stan magazynu w każdym okresie jest równy stanowi magazynu w poprzednim okresie plus produkcji minus sprzedaży. 
o_zmiany_produkcji1{t in 2..okres}: produkcja[t] - produkcja[t-1] <= produkcja[t-1] * maksymalna_zmiana_produkcji;	  	# Ogranicza zmienność produkcji między okresami do wartości nie przekraczającej maksymalnej zmiany produkcji w procentach.
o_zmiany_produkcji2{t in 2..okres}: - maksymalna_zmiana_produkcji * produkcja[t-1] <= (produkcja[t] - produkcja[t-1]); 	# Ogranicza zmienność produkcji między okresami do wartości nie przekraczającej maksymalnej zmiany produkcji w procentach.
