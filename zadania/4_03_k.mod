...
# Ograniczenia 
o_wagi: sum{i in 1..przedmioty} czy_zabrac_przedmiot[i] * wagi_przedmiotow[i] <= maksymalna_waga;   # Suma wag wszystkich wybranych przedmiotów nie może przekroczyć maksymalnej wagi paczki. Ma to zapobiec sytuacji, w której paczka byłaby zbyt ciężka.
o_para_L1{(i,j) in para_L1}: czy_zabrac_przedmiot[i] + czy_zabrac_przedmiot[j] <= 1;  				# Każda para przedmiotów z L1 może zawierać tylko jeden z tych przedmiotów w paczce.
o_para_L2{(i,j) in para_L2}: czy_zabrac_przedmiot[i] + czy_zabrac_przedmiot[j] >= 1;  				# Każda para przedmiotów z L2 musi zawierać przynajmniej jeden z tych przedmiotów w paczce.
o_para_L3{(i,j) in para_L3}: czy_zabrac_przedmiot[i] <= czy_zabrac_przedmiot[j];  	 				# Dla każdej pary przedmiotów z L3, jeśli jeden z tych przedmiotów zostanie wybrany do paczki, drugi musi również zostać wybrany lub już nie zostać wybrany. 
...
param wagi_przedmiotow:= 	1 7
							...
							19 6
							20 4;
param wartosci_przedmiotow:=1 7
							2 2
							3 10
							4 2
							5 5
