...
o_obserwowania {(wiersz, kolumna) in paczki}:
    sum{(k, kolumna) in pola: k >= wiersz and k <= wiersz + zasieg} (if k >= 1 and k <= powierzchnia then czujnik[k,kolumna]) +
    sum{(wiersz, k) in pola: k >= kolumna - zasieg and k <= kolumna} (if k >= 1 and k <= powierzchnia then czujnik[wiersz,k]) >= 1;
    
data;
param powierzchnia := 9;
param zasieg := 3;
set paczki := 3 4 3 5 3 6 4 3 4 7 5 3 5 7 6 3 6 7 7 4 7 5 7 6;
