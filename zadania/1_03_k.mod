...
# Zmienne decyzyjne
var kg_kotlety{i in 1..typ} >= 0, <= limit_kotlety[i], integer;   # Liczba kilogramów kotletów do wyprodukowania dla każdego typu, ograniczona przez limit sprzedaży.
var liczba_indykow{i in 1..typ} >= 0, integer;                    # Liczba zakupionych indyków dla każdego typu.
...
o_mieso_biale: sum{i in 1..typ} liczba_indykow[i] * mieso_biale[i] >= sum{i in 1..typ} kg_kotlety[i] * proc_miesa_bialego[i]; 		  # Wymagana ilość mięsa białego musi być większa lub równa sumie ilości mięsa białego w kotletach każdego typu.
o_mieso_ciemne: sum{i in 1..typ} liczba_indykow[i] * mieso_ciemne[i] >= sum{i in 1..typ} kg_kotlety[i] * (1 - proc_miesa_bialego[i]); # Wymagana ilość mięsa ciemnego musi być większa lub równa sumie ilości mięsa ciemnego w kotletach każdego typu.

data;
param typ := 2;
param: limit_kotlety cena_kotlet koszt_indyk proc_miesa_bialego mieso_biale mieso_ciemne :=	1 	50 	8 	10 	0.7 	3 		2
    																						2 	30 	6 	8 	0.5 	1.5 	4;
solve;
