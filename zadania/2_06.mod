...
var ilosc_odpadow{miasto, spalarnia} >= 0; 
...
o_limit_odpady{i in miasto}: sum{j in spalarnia} ilosc_odpadow[i,j] = odpady[i]; 	
# Nakłada limit na ilość odpadów, które mogą być spalane w każdej z spalarni.					
o_limit_spalarnia{j in spalarnia}: sum{i in miasto} ilosc_odpadow[i,j] <= limit_spalania[j];
# Określa limit ilości popiołu, który może być składowany w każdym z wysypisk. 							
o_limit_wysypiska{k in wysypisko}: sum{j in spalarnia} ilosc_popiolu[j,k] <= limit_wysypiska[k];
# Wymusza, że ilość popiołu wytwarzana w każdej spalarni musi być równa ilości odpadów, które zostały spalone.			
o_spalanie{j in spalarnia}: sum{i in miasto} ilosc_odpadow[i,j] * wsp_spalania = sum{k in wysypisko} ilosc_popiolu[j,k];

data;
param wsp_spalania := 0.3; 
param koszt_transportu := 3; 
param: miasto: odpady:= 'Miasto-1' 1500 'Miasto-2' 1400;
param: spalarnia: limit_spalania, koszt_spalania:= 'Spalarnia-A' 1500 40 'Spalarnia-B' 1500 30;
param: wysypisko: limit_wysypiska:= 'Wysypisko-1' 	500'Wysypisko-2' 	500;
